...
DB_TABLE_NAME = None # getenv('DB_TABLE_NAME')
DB_COLUMN_NAMES = None
"""
{ # <local name>:<name in db>
    'battery': getenv('DB_COLUMN_NAME_BATTERY')
    'cpu': getenv('DB_COLUMN_NAME_CPU')
    'activity': getenv('DB_COLUMN_NAME_ACTIVITY')
    'point': getenv('DB_COLUMN_NAME_POINT')
    'orientation': getenv('DB_COLUMN_ORIENTATION')
...
class RobotStatusSub(BaseStatusSub):
    def __init__(self):
        ...
        # subscriptions
        self.subscription_dict['battery'] = self.create_subscription(RobotBattery, 'robot_battery', self.subscription_callback, 3)
        self.subscription_dict['cpu'] = self.create_subscription(RobotCpu, 'robot_cpu', self.subscription_callback, 3)
        self.subscription_dict['activity'] = self.create_subscription(RobotActivity, 'robot_activity', self.subscription_callback, 3)
        self.subscription_dict['point'] = self.create_subscription(RobotPoint, 'robot_point', self.subscription_callback, 3)
        self.subscription_dict['orientation'] = self.create_subscription(RobotQuaternion, 'robot_orientation', self.subscription_callback, 3)
        self.subscription_dict['misc'] = self.create_subscription(RobotMisc, 'robot_misc', self.subscription_callback, 3)
