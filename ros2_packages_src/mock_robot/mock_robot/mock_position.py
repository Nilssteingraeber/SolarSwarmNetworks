...
class MockPosition(object):
    ...
    def points(self, p):
        if type(p) == type(self.__points) and len(p) > 0: # must be (not empty) list
            for point in p:
                if type(point).__name == 'ndarray': # each element must be of numpy.ndarray
                    if len(point) == len(self.__points) and type(point[0]).__name__ == 'float64': # each element must have same dimension and content type (content type identical across an ndarray)
                        self.__points = p
                        return
