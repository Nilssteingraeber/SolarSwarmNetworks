...
const COLOR_CACHE = new Map<string, { lastCalc: number, color: Color }>();
const COLOR_THROTTLE_MS = 3500;

const getDroneColor = (nid: string) => {
    const now = performance.now();
    const cached = COLOR_CACHE.get(nid);

    if (cached && (now - cached.lastCalc < COLOR_THROTTLE_MS)) {
        return cached.color;
    }
    ...
    if (history && history.length > 0) {
        ...
        const lagSeconds = Math.abs(currentTime - heardMs) / 1000.0;

        if (lagSeconds < 10) result = Color.GREEN;
        else if (lagSeconds < 15) result = Color.YELLOW;
        else if (lagSeconds < 25) result = Color.RED;
    }

    COLOR_CACHE.set(nid, { lastCalc: now, color: result });
    return result;
}
    ...
    actions: {
        ...
        async drawFlightPath(nid: string, aroundTime?: number) {
            ...
            for (let i = 0; i < entries.length; i++) {
                ...
                const progress = 1.0 - (ageMs / 60000);

                const safeProgress = Math.max(0, Math.min(1, progress));

                const hue = 0.65 * (1.0 - safeProgress);
